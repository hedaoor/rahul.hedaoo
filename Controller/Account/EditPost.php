...
class EditPost extends \Magento\Customer\Controller\AbstractAccount
{
        ...
        if ($validFormKey && $this->getRequest()->isPost()) {
            ...
            try {
                ...
                if(!$isPasswordChanged){
                    return $resultRedirect->setPath('customer/account/edit/changepass/1/');
                }
                ...
                $this->messageManager->addSuccess(__('Your account information has been updated.'));
    ...
    private function populateNewCustomerDataObject(
        ...
        /** Fixed issue for, while updating customer information from customer account section, 
         * It was updating all customer's group to default 'conusmer' group 
         * We added getCustomerMapper() Magento Core moethod to fix this issue
         */
    ...
    protected function changeCustomerPassword($email)
    {
        ...
        if ($this->getRequest()->getParam('change_password')) {
            ...
            $data['current_password'] = $this->getRequest()->getPost('current_password');
            $data['password'] = $this->getRequest()->getPost('password');
            $data['password_confirmation'] = $this->getRequest()->getPost('password_confirmation');

            if($this->_pass_helper->isEnable()){
                
                if(!$this->_pass_helper->changePassword($data,'customer_account_edited',null))
                {
                     $this->messageManager->addError('New password cannot be the same as old password');
                     return 0;
    ...
    /**
     * This method will fix consumer group updation issue while updating customer information in my account section
